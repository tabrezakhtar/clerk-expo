...
export default function Page() {
  ...
  const handleSubmit = async () => {
    console.log('handleSubmit', { emailAddress, passwordLength: password.length, status: signIn.status, fetchStatus })
    ...
    if (signIn.status === 'complete') {
      ...
      // If the user has second-factor enabled (MFA), send a code for them to verify.
      // This happens even when credentials are correct.
  ...
  const handleVerifySecondFactor = async () => {
    ...
    if (emailCodeFactor) {
      ...
      console.error('No supported second factor available', signIn.supportedSecondFactors)
      return
    }
    ...
    return (
      <ThemedView style={styles.container}>
        ...
        <ThemedText style={styles.label}>
          {emailCodeFactor
            ? 'A code has been sent to your email. Enter it below to continue.'
            : phoneCodeFactor
            ? 'A code has been sent to your phone. Enter it below to continue.'
    ...
    <ThemedView style={styles.container}>
      ...
      {signIn?.status && (
        <ThemedText style={styles.debug}>
          signIn.status: {signIn.status} — fetchStatus: {fetchStatus}
        </ThemedText>
      )}
